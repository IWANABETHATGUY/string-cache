...
pub(crate) struct Set {
    buckets: Box<[Mutex<Option<Box<Entry>>>; NB_BUCKETS]>,
}
...
pub(crate) static DYNAMIC_SET: Lazy<Set> = Lazy::new(|| {
    ...
    Set {
        buckets: unsafe { Box::from_raw(vec.as_ptr() as *mut [Mutex<T>; NB_BUCKETS]) },
    }
    // })
});
...
impl Set {
    pub(crate) fn insert(&self, string: Cow<str>, hash: u32) -> NonNull<Entry> {
        ...
        {
            let mut guard = self.buckets[bucket_index].lock();
            let mut ptr = guard.as_mut();
        ...
        let mut entry = Box::new(Entry {
            next_in_bucket: { self.buckets[bucket_index].lock().take() },
            hash,
            ref_count: AtomicIsize::new(1),
            string: string.into_boxed_str(),
        });
        let ptr = NonNull::from(&mut *entry);
        *self.buckets[bucket_index].lock() = Some(entry);
    ...
    pub(crate) fn remove(&self, ptr: *mut Entry) {
        ...
        let mut current: &mut Option<Box<Entry>> = &mut self.buckets[bucket_index].lock();
